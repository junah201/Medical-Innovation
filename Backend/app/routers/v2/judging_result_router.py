...
def create_judging_result(judging_result_create: schemas_v2.JudgingResultCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
@router.get("/{judging_event_id}/all", response_model=schemas_v2.JudgingResultList)
def get_judging_results(judging_event_id: int, skip: int = 0, limit: int = 40, db: Session = Depends(get_db)):
    ...
    return schemas_v2.JudgingResultList(
